    ...
    *,
    ...
    if ocr_confidence < 0.55:
        ...
        flags.append(
            _flag(
                ...
                "warn",
                f"OCR confidence {ocr_confidence:.2f} — manual review recommended",
                "ocr",
            )
        )
    ...
    for required in ("serial_number", "parcel_number", "sheet_number"):
        if not parsed.get(required):
            ...
            flags.append(
                _flag(
                    f"missing_{required}", "warn", f"Field '{required}' could not be parsed", "ocr"
                )
            )
    ...
    if history is not None and history.get("status") == "ok":
        ...
        if data.get("disputes"):
            ...
            flags.append(
                _flag(
                    "active_dispute",
                    "critical",
                    f"{len(data['disputes'])} active dispute(s) recorded",
                    "transaction_history",
                )
            )
        if data.get("encumbrances"):
            ...
            for enc in data["encumbrances"]:
                etype = enc.get("type")
                lender = enc.get("lender", "")
                reg = enc.get("registered_at", "")
                flags.append(
                    _flag(
                        "encumbrance",
                        "high",
                        f"Active {etype} — {lender} ({reg})".strip(),
                        "transaction_history",
                    )
                )
